...
Optionally, if one wants to add Poisson noise to the data, then a :samp:`{TEXPTIME}` keyword must be present in the header of the loaded data.
...
.. code-block:: python
    ...
    file2 = f'example/{name}_105_PSF2.fits'
    var   = f'example/{name}_105_variance.fits'
    zpt   = 26.27
    
    f1    = SED.Filter(name, file, file2, var, zpt, ext=0, extErr=0)
    
Five parameters have to be provided:

* **name** : Name of the filter. This name will be used later on when constructing the data table for the SED fitting code.
* **file** : Data file name
* **file2** : File name for the data convolved by the square of the PSF. This is used to add Poisson noise into the variance map. If it is already present, then provide :python:`None` instead of a file name.
...
Optionally, if the data and/or variance maps have multiple extensions, one can provide the extension number with the :python:`ext` and :python:`extErr` keywords.
...
By default, when the :py:class:`~.FilterList` object is constructed for the first time, it automatically calls :py:meth:`~.FilterList.setCode` method, which builds an `Astropy Table`_ with the data required by the given SED fitting code.
...
2. Opptionally: add Poisson noise to the variance map
...
If needed, the table can easily be retrieved as follows
...
* **cleanMethod**: :py:attr:`~.CleanMethod.ZERO` (to put 0 if data is in flux unit or -99 if in magnitude on bad pixels) or :py:attr:`~.CleanMethod.MIN` (to put the minimum value of the good pixels on bad pixels)
...
.. code-block:: python
    ...
    flist.setCode(SEDcode.LEPHARE, cleanMethod=CleanMethod.ZERO, scaleFactor=100, texpFac=0)
...
.. code-block:: python
    ...
    cat = flist.toCatalogue(*args, **kwargs) # Replace *args and **kwargs by the arguments and keyword arguments specific to the catalogue
...
The input file creation and the interface allowing to run the SED fitting codes directly from python are all dealt with by a :py:class:`~.SED` object.
...
.. code-block:: python
    ...
    output = sed(cat, outputParams=params, **kwargs) # Replace **kwargs by the keyword arguments specific to the SED object
...
When the :py:class:`~.Output` object is created, data is loaded from the output file into an `Astropy Table`_ object. One can access the entire table or simply a column as follows:
...
.. warning::
    ...
    The data in the table are raw, that is they have not been scaled back if a scaling was applied (only applies for LePhare). Since the scaling must only be applied for extensive quantities (such as mass), the data is not scaled back by default.
    
    To scale back the data, please use :py:meth:`~.Output.toImage` method with the correct 
